...
const PI_VERSION_MAX_BUFFER = 16 * 1024;

export function probePiVersion(piBin, env = process.env, options = {}) {
  ...
  return new Promise((resolvePromise, reject) => {
    execFileImpl(
      ...
      ["--version"],
      {
        encoding: "utf8",
        env,
        timeout: options.timeoutMs ?? PI_VERSION_TIMEOUT_MS,
        maxBuffer: PI_VERSION_MAX_BUFFER,
        shell: false,
      },
      (error, stdout, stderr) => {
        if (error) {
          const detail = stderr?.trim() || error.message;
          reject(new Error(`Pi version probe failed for ${piBin}: ${detail}`));
          return;
        }
...
function piVersionError(expected, actual, piBin) {
  return new Error(
    `Pi version mismatch for ${piBin}: expected exactly ${expected}, found ${actual || "<empty>"}. ` +
      "Run pi-doctor, then use the managed Pi upgrade workflow before retrying.",
  );
}
...
export async function createLaunchPlan(profileId, passthroughArgs = [], options = {}) {
  ...
  let piVersion;
  try {
    piVersion = await (options.versionProbe ?? probePiVersion)(paths.piBin, env);
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Unable to verify pinned Pi ${catalog.pi.version} at ${paths.piBin}: ${detail}. ` +
        "Run pi-doctor, then use the managed Pi upgrade workflow before retrying.",
    );
  }
  if (piVersion !== catalog.pi.version) {
    throw piVersionError(catalog.pi.version, piVersion, paths.piBin);
  }
  ...
  return {
    ...
    env: buildProfileEnvironment(profileId, paths, prepared, env),
  };
}
