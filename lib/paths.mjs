...
export function resolveHarnessPaths(env = process.env, overrides = {}) {
  ...
  const rawPodmanBin = overrides.podmanBin ?? env.PI_HARNESS_PODMAN_BIN ?? "podman";
  const podmanBin = rawPodmanBin.includes("/") || rawPodmanBin.includes("\\") ? resolve(rawPodmanBin) : rawPodmanBin;
  const rawSystemdRunBin = overrides.systemdRunBin ?? env.PI_HARNESS_SYSTEMD_RUN_BIN ?? "systemd-run";
  const systemdRunBin =
    rawSystemdRunBin.includes("/") || rawSystemdRunBin.includes("\\")
      ? resolve(rawSystemdRunBin)
      : rawSystemdRunBin;
  const cellsDir = directoryFromEnv(
    overrides.cellsDir ?? env.PI_HARNESS_CELLS_DIR,
    join(homeDir, ".local", "state", "pi-harness", "cells"),
  );
  const cellImage = (overrides.cellImage ?? env.PI_HARNESS_CELL_IMAGE ?? "localhost/pi-harness-cell:0.1.0").trim();
